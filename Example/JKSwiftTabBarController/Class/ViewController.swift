...
class ViewController: UIViewController, JKTabBarItemRepeatTouch {
    ...
    @objc func number(btn: UIButton) {
        ...
        JK.mainViewController.showBadgeNumber(btn.isSelected ? 2000 : 0, index: 4)
    }

    @objc func changeIndex(btn: UIButton) {
        JK.mainViewController.setSelectedItem(at: 3)
    ...
    @objc func changeRedClick(btn: UIButton) {
        btn.isSelected = !btn.isSelected
        if btn.tag == 100 {
            JK.mainViewController.setRedPoint(at: 3, isShow: btn.isSelected ? true : false)
            JK.mainViewController.setRedPoint(at: 4, isShow: btn.isSelected ? true : false)
        } else {
